...
# endif
    ...
    if ((accept_get && strncmp(meth, "GET ", 4) == 0)
        ...
        const char *const http_version_str = " "HTTP_PREFIX_VERSION;
        size_t http_version_str_len = strlen(http_version_str); /* " HTTP/1." */
...
{
    int ret = BIO_printf(cbio, HTTP_PREFIX_VERSION"0 200 OK\r\n%s" /* HTTP/1.0 */
                         "Content-type: %s\r\n"
                         "Content-Length: %d\r\n\r\n",
