...
struct ossl_http_req_ctx_st {
    ...
    unsigned char *readbuf;     /* Buffer for reading request or response */
                                       ...
                                       const char *path)
{
    ...
    if (BIO_printf(rctx->mem, "%s "HTTP_PREFIX_VERSION"0\r\n", path) <= 0)
        return 0;
    rctx->resp_len = 0;
                                         ...
                                         const char *content_type, BIO *req)
{
    ...
    rctx->req = req;
    if ((req_len = BIO_get_mem_data(req, &req)) <= 0)
...
static OSSL_HTTP_REQ_CTX *http_req_ctx_new(int free_wbio, BIO *wbio, BIO *rbio,
                                           ...
                                           const char *proxy,
                                           const char *server, const char *port,
                                           int maxline, int overall_timeout)
{
 ...
 next_io:
    ...
    case OHS_WRITE_REQ:
        ...
        if (rctx->req != NULL && !BIO_eof(rctx->req)) {
            n = BIO_read(rctx->req, rctx->readbuf, rctx->readbuflen);
            if (n <= 0) {
                if (BIO_should_retry(rctx->rbio))
                    return -1;
                ERR_raise(ERR_LIB_HTTP, HTTP_R_FAILED_READING_DATA);
...
#define BUF_SIZE (8 * 1024)
    ...
    BIO_printf(fbio, "CONNECT %s:%s "HTTP_PREFIX_VERSION"0\r\n", server, port);
