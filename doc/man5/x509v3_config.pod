...
copy the subject key identifier (SKID) from the issuer certificate
(which is the same as the current certificate if it is self-issued), where
for self-signed certificates the hash of the public key is taken as fallback
if the option B<always> is present.
