...
[B<-policy arg>]
[B<-keyfile arg>]
[B<-keyform PEM|DER|P12|ENGINE>]
[B<-key arg>]
[B<-passin arg>]
[B<-cert file>]
[B<-certform PEM|DER|P12>]
[B<-selfsign>]
[B<-in file>]
[B<-out file>]
[B<-notext>]
...
The CA private key to sign requests with.

=item B<-keyform PEM|DER|P12|ENGINE>
...
This option is deprecated; better use B<--passin>.

=item B<-passin arg>

The password source for key or certificate files.
...
=item B<-certform PEM|DER|P12>

The format of the data in certificate files.
The default is PEM.

=item B<-pkcs12in file>

The file holding the CA private key and certificate in PKCS#12 format.
This implies B<-keyfile file -cert file -keyform P12 -certform P12>.
