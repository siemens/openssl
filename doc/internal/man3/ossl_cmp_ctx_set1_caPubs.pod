...
ossl_cmp_ctx_set0_next_recipient,
ossl_cmp_ctx_set1_recipient_from_sender
...
=head1 SYNOPSIS
 ...
 int ossl_cmp_ctx_set1_recipient_from_sender(OSSL_CMP_CTX *ctx,
                                             const OSSL_CMP_MSG *src);
...
ossl_cmp_ctx_set0_next_recipient() sets the next_recipient field of the context
to B<val>, freeing any previously stored next_recipient.
Ownership of B<val> is transferred to B<ctx> (unless B<val> is NULL).

ossl_cmp_ctx_set1_recipient_from_sender() extracts the sender GENERAL_NAME
from the header of the message B<src> (if present) and stores a copy of it in the
next_recipient field of B<ctx> via ossl_cmp_ctx_set0_next_recipient().
If B<src> or its header or sender field is NULL, no error is raised.
