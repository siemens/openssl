...
The recipient field is set to a copy of B<ctx->next_recipient> if present,
supporting any GENERAL_NAME choice. Otherwise, it is determined from
B<ctx->recipient>, B<ctx->srvCert>, B<ctx->issuer>, B<ctx->oldCert>, or B<ctx->cert>
via ossl_cmp_hdr_set1_recipient().
