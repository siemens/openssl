...
It also clears any ITAVs added by calling OSSL_CMP_CTX_push0_genm_ITAV().
...
OSSL_CMP_CTX_reset_geninfo_ITAVs()
clears any ITAVs added by calling OSSL_CMP_CTX_push0_geninfo_ITAV().
...
B<OSSL_CMP_PKISTATUS_accepted> on sucessful receipt of a GENP message,
B<OSSL_CMP_PKISTATUS_request>
if an IR/CR/KUR/RR/GENM request message could not be produced,
B<OSSL_CMP_PKISTATUS_trans>
on a transmission error or transaction error for this type of request, and
B<OSSL_CMP_PKISTATUS_unspecified>
...
else B<OSSL_CMP_PKISTATUS_unspecified>.
