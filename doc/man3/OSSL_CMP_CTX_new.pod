...
OSSL_CMP_CTX_set1_recipient() sets in I<ctx> the recipient name to use
in the PKIHeader of the initial CMP request message of a transaction.
This usually is the the X509 name of the (CA) server being addressed.
For each subsequent message of a transaction,
the sender of the previous message will be used instead.
This implements the requirements of RFC 9483 section 3.1.
...
The B<OSSL_CMP_OPT_NO_CACHE_EXTRACERTS> option as well as
OSSL_CMP_CTX_get0_geninfo_ITAVs() were added in OpenSSL 3.3.
