...
or the single-shot MAC function L<EVP_mac(3)>.

HMAC() can continue to be used by defining B<OPENSSL_COMPENSATE_DEPRECATION>.
Calls to

    HMAC(evp_md, key, keylen, data, datalen, out, outlen)

where I<out> is not NULL and has sufficient space can also be replaced by
