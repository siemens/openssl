...
OSSL_HMAC() computes the message authentication code of the B<datalen> bytes
at B<data> using the hash function B<digest>
and the key B<key> which is B<key_len> bytes long.
If I<out> is not NULL, it places the result in the memory pointed at by I<out>,
but only if I<outsize> is at least the output size of the digest function,
which is no more than B<EVP_MAX_MD_SIZE> (otherwise no computation is made).
...
or the more convenient functions L<EVP_MAC_calc(3)> or OSSL_HMAC().
Calls to

    HMAC(evp_md, key, keylen, data, datalen, out, outlen)

where I<out> is not NULL and has sufficient space can be replaced by

    OSSL_MAC(NULL, NULL, EVP_MD_name(evp_md), key, keylen,
             data, datalen, out, EVP_MD_size(evp_md), outlen)

HMAC is a MAC (message authentication code), i.e. a keyed hash
function used for message authentication, which is based on a hash
function.

HMAC() computes the message authentication code of the I<n> bytes at
I<d> using the hash function I<evp_md> and the key I<key> which is
...
I<evp_md> is a message digest such as EVP_sha1(), EVP_ripemd160() etc. HMAC does
...
OSSL_HMAC() and HMAC() return a pointer to the message authentication code
...
EVP_MAC_calc(), L<SHA1(3)>, L<evp(7)>

=head1 HISTORY

OSSL_HMAC() was added in OpenSSL 3.0.

All other functions were deprecated in OpenSSL 3.0.
