...
EVP_MAC_CTX_get_mac_size, EVP_MAC_calc, OSSL_Q_mac,
EVP_MAC_init, EVP_MAC_update, EVP_MAC_final,
...
OSSL_Q_mac() is a front-end to  EVP_MAC_calc() fetching the MAC algorithm
I<mac_name> using any given I<libctx> and property query string I<propq>.

EVP_MAC_calc() computes the message authentication code of the I<data> with
length I<datalen> using the MAC algorithm I<mac> with its I<params>
(which may be NULL if not needed), and the key I<key> with length I<keylen>.
...
OSSL_Q_mac() and EVP_MAC_calc() return a pointer to the computed MAC value,
or NULL on error.
