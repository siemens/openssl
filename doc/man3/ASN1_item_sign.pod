...
I<signature>, <algor1>, and I<algor2> are essentially output parameters, as follows.
...
Each of the <algor1> and I<algor2> parameters may be NULL,
otherwise the B<X509_ALGOR> structure being pointed to may have been allocated
using I<X509_ALGOR_new()> or indirectly as part of an ASN.1 structure,
like for the B<X509>fields I<sig_alg> and I<cert_info.signature>.
On success, ASN1_item_sign_ex() assigns to the structure the signature algorithm
before generating the signature value and setting it into I<signature>.
On error, due to the problematic "reuse" behaviour of L<d2i_X509_ALGOR(3)>
being applied here, the B<X509_ALGOR> structure pointed at is freed internally.
So to prevent double free, the caller should on error
assign NULL to the pointer variable(s) used to pass in <algor1> and/or I<algor2>.
