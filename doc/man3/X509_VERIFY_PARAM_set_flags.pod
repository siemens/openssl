...
(trusted) certificate in a chain, which typically is a self-signed root CA
...
The B<X509_V_FLAG_PARTIAL_CHAIN> flag causes non-self-signed (i.e.,
intermediate or just self-issued) certificates in the trust store to be treated
as trust anchors, in the same way as self-signed "root" CA certificates.
