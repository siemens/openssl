...
OSSL_CRMF_ENCRYPTEDVALUE_decrypt() decrypts the encryptedValue I<env>,
using the private key I<pkey>, the library context I<libctx>,
and property query string I<propq> (see L<OSSL_LIB_CTX(3)>).
It places the length of the decrypted data, or 0 on error, in I<*outlen>
and returns a pointer to the decrypted data, or NULL on error.
...
I<libctx> and property query string I<propq>.
...
OSSL_CRMF_ENCRYPTEDKEY_get1_encCert() is like
OSSL_CRMF_ENCRYPTEDVALUE_get1_encCert() but uses instead of
OSSL_CRMF_ENCRYPTEDVALUE the more general input type OSSL_CRMF_ENCRYPTEDKEY.
...
OSSL_CRMF_ENCRYPTEDKEY_get1_encCert() were added in OpenSSL 3.1.
