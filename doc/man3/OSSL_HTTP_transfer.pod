...
Any given C<http://> or C<https://> prefix in the I<server> argument
must be consistent with the I<use_ssl> argument.
The server port to connect to can be given explicitly via the I<port> parameter,
but if I<port> is NULL and the hostname in I<server> is followed by C<:>
it is taken from the string thereafter.
Any other URL components potentially given with the I<server> are ignored.
The function creates a network BIO internally using L<BIO_new_connect(3)>
for connecting to the given server at the given port,
defaulting to C<80> for HTTP or C<443> for HTTPS.
