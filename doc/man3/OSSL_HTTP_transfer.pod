...
An ASN.1 DER-encoded response is expected by OSSL_HTTP_get_asn1() and
OSSL_HTTP_post_asn1(), while for OSSL_HTTP_get() or OSSL_HTTP_transfer()
this is only the case if the I<expect_asn1> parameter is 1.
If it is 0 then an ASN.1 response is forbidden, while -1 means it is allowed.
