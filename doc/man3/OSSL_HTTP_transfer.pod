...
OSSL_HTTP_get() uses HTTP GET to obtain data from the I<bio> if non-NULL,
else from the server contained in the I<url>, and returns it as a memory BIO.
If I<bio> is non-NULL, any host and port components in the I<url> are not used
for connecting but the hostname is use for the C<Host> header.
...
OSSL_HTTP_get_asn1() is like OSSL_HTTP_get() but in addition parses the received
contents (representing, e.g., an X.509 certificate) as an ASN.1 DER-encoded
value with structure type as specified by I<rsp_it>.

OSSL_HTTP_transfer_asn1() exchanges an HTTP request and response
over a connection managed via I<prctx> without supporting redirection.
It combines OSSL_HTTP_open(), OSSL_HTTP_set_request(), OSSL_HTTP_transfer(),
and OSSL_HTTP_open().
If I<req> is NULL it uses the HTTP GET method to send the request,
else HTTP POST with the contents of I<req> of type I<req_it>
and the optional I<content_type>.
It expects a response in ASN.1 encoding of type I<tsp_it>.
...
On success, OSSL_HTTP_transfer() and OSSL_HTTP_get()
return a memory BIO containing the data received.
Similarly, OSSL_HTTP_transfer_asn1(),
OSSL_HTTP_get_asn1() and OSSL_HTTP_post_asn1()
return an B<ASN1_VALUE> containing the response structure received.
