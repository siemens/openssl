...
CMS_verify, CMS_SignedData_unwrap_verify,
CMS_set1_signers_certs, CMS_get0_signers
...
=head1 SYNOPSIS
 ...
 int CMS_verify(CMS_ContentInfo *cms, STACK_OF(X509) *scerts, X509_STORE *store,
...
CMS_verify() verifies a CMS SignedData structure. I<cms> is the CMS_ContentInfo
structure to verify. I<scerts> is a set of certificates in which to search for
the signing certificate(s). I<store> is a trusted certificate store used for
chain verification.
I<detached_data> holds the content if the content is not present in I<cms>.
The content is written to I<out> if it is not NULL.

CMS_SignedData_unwrap_verify()
unwraps a CMS SignedData structure I<sd>, consumes it, and returns its content.
Doing so, it verifies the CMS signature as described for CMS_verify(),
using any given I<extra> certificates for chain building.

I<flags> is an optional set of flags, which can be used to modify the verify
operation.

CMS_set1_signers_certs() is used by CMS_verify().
It attempts to complete the list of signing certificates given in the
CMS ContentInfo I<cms>, using the list of certificates <scerts>.
It uses also the extra certs in I<cms> unless I<flags> contains B<CMS_NOINTERN>.
...
the I<scerts> parameter (if it is not NULL) and then looking in any
...
If B<CMS_NOINTERN> is set the extra certificates in the message itself are not
searched when locating the signing certificate(s). This means that all the
signing certificates must be in the I<scerts> parameter.
...
in the I<scerts> parameter. In this case if the signer is not one of the
certificates supplied in I<scerts> then the verify will fail because the
...
CMS_verify() returns 1 on verificaton success and zero if an error occurred.

CMS_SignedData_unwrap_verify() returns NULL on error,
otherwise a BIO containing the signed content.

CMS_set1_signers_certs() returns the number of certificates added, -1 on error.
...
CMS_SignedData_unwrap_verify() was added in OpenSSL 3.1.
