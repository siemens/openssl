...
for intitiating delayed delivery and cleaning up a transaction.
I<delay> takes a custom server context and a request message as input
and must return 1 if delayed deliivery of the responpse shall be initiated,
0 if not, and -1 on error.
I<clean> takes a custom server context and a transaction ID pointer as input.
If this pointer is NULL a new transaction is being started.
Otherwise the transaction with the given ID is being terminated,
so it should reset the related portions of the state and free related memory.
It return 1 on success and 0 on error.
