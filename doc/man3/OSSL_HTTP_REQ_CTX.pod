...
which should be 1 to indicate the HTTP POST method or 0 to indicate HTTP GET.
...
When requesting "keep-alive", i.e., the I<flags> parameter contains
B<OSSL_HTTP_FLAG_ENABLE_KEEP_ALIVE> or B<OSSL_HTTP_FLAG_REQUIRE_KEEP_ALIVE>,
due to the structure of an HTTP request,
the function must be used before calling OSSL_HTTP_REQ_CTX_set1_req().
...
If the I<asn1> parameter is nonzero, which has the same effect as
including B<OSSL_HTTP_FLAG_EXPECT_ASN1> in the I<flags> parameter,
a structure in ASN.1 encoding will be
...
This is the default.
I<timeout> < 0 takes over any value set via the I<timeout> argument of
...
Including B<OSSL_HTTP_FLAG_ENABLE_RETRY> causes the high-level functions
L<OSSL_HTTP_get(3)>, L<OSSL_HTTP_get_ex(3)>, and L<OSSL_HTTP_transfer_ex(3)>
...
It is available until I<rctx> is deallocated, using OSSL_HTTP_REQ_CTX_free()
or L<OSSL_HTTP_close(3)>.
...
OSSL_HTTP_get_status() returns the last HTTP response status code received.

OSSL_HTTP_is_alive() can be used to query if the HTTP connection
given by I<rctx> is still alive, i.e., I<rctx> is not NULL,
the client requested (or even required) a persistent connection,
and the server did not disagree on keeping the connection open.

If the client application requested or required a persistent connection
and this was granted by the server, it can use I<rctx> as long as it wants
...
In case the client application retains I<rctx> but the connection then dies
...
OSSL_HTTP_may_retry() can be used to query if the last request may be retried,
i.e., this has been enabled for the client and a non-fatal HTTP response status
...
OSSL_HTTP_get_status(), OSSL_HTTP_is_alive(), and OSSL_HTTP_may_retry()
return the value specified above if the I<rctx> argument is non-NULL, else 0.
...
L<OSSL_HTTP_open(3)>,
L<OSSL_HTTP_FLAG_ENABLE_KEEP_ALIVE(3)>,
L<OSSL_HTTP_FLAG_REQUIRE_KEEP_ALIVE(3)>,
L<OSSL_HTTP_FLAG_ENABLE_RETRY(3)>,
L<OSSL_HTTP_FLAG_EXPECT_ASN1(3)>,
L<OSSL_HTTP_set1_request(3)>,
L<OSSL_HTTP_get(3)>,
L<OSSL_HTTP_get_ex(3)>,
L<OSSL_HTTP_transfer(3)>,
L<OSSL_HTTP_transfer_ex(3)>,
L<OSSL_HTTP_close(3)>

=head1 HISTORY

The functions OSSL_HTTP_get_status() and OSSL_HTTP_may_retry()
