...
=head1 SYNOPSIS
 ...
 BIO *OSSL_HTTP_i2d(const ASN1_ITEM *it, const ASN1_VALUE *val);
...
I<wbio>), and the maximum expected response header line length I<maxline>.
A value <= 0 less indicates that
...
in order to exchange an request and response or until a timeout is reached.
...
OSSL_HTTP_d2i() DER decodes the response of an HTTP transfer
using the ASN.1 template I<it>.
It consumes and frees the response provided as the BIO pointer argument I<res>.
If I<res> is NULL it returns NULL.
...
OSSL_HTTP_REQ_CTX_set1_req(), OSSL_HTTP_REQ_CTX_set_expected(),
and OSSL_HTTP_REQ_CTX_nbio()
...
OSSL_HTTP_d2i() returns a pointer to a B<ASN1_VALUE>, or NULL on error.
