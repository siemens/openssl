...
which may be NULL for no expectation), and an
indicator whether the response is expected to be a DER encoded ASN.1 structure
(I<expect_asn1>; 1 means required, 0 means forbidden, -1 means allowed).
...
It sets the expectation that also the response is a DER encoded ASN.1 structure.
...
OSSL_HTTP_REQ_CTX_is_asn1() returns 1 if the response is ASN.1 encoded,
0 if it is not, and -1 if we do not know because the server response was
insufficient, or -2 of an error occurred.
...
OSSL_HTTP_REQ_CTX_is_asn1() returns an integer as described above.
