...
The I<method_POST> argument may be 1 to indicate that the C<POST>
method is to be used, or 0 to indicate that the C<GET> method is to be used.
The HTTP I<server> and its I<port> must be given if the request shall go through
an HTTP (but not HTTPS) proxy, otherwise they should be left NULL.
