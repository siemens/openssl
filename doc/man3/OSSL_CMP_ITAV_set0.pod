...
OSSL_CMP_ITAV_new0_caCerts() creates an B<OSSL_CMP_ITAV> structure with
infoType B<NID_id_it_caCerts> and fills in the provided list of certificates.
The I<caCerts> argument may be NULL or contain any number of certificates.
On success, ownership of I<caCerts> is transferred to the ITAV structure.

OSSL_CMP_ITAV_get0_caCerts() requires that
the infoValue in I<itav> has infoType B<NID_id_it_caCerts>.
It returns in I<*out> NULL if there are no CA certificates in I<itav>, otherwise
...
OSSL_CMP_ITAV_create() and OSSL_CMP_ITAV_new0_caCerts()
...
OSSL_CMP_ITAV_push0_stack_item() and OSSL_CMP_ITAV_get0_caCerts()
...
OSSL_CMP_ITAV_get0_caCerts() was added in OpenSSL 3.1.
