...
B<NULL>. Yet note that L<X509_verify_cert(3)> will need a verification target.
This can also be set using X509_STORE_CTX_set_cert(), or by default
the first element of the list of untrusted certificates is taken if present.
