...
only good for one call to L<X509_verify_cert(3)>; if you want to verify a second
...
and a set of additional certificates may be provided in I<untrusted>,
which will be untrusted but may be used to build the chain.
Any or all of the I<trust_store>, I<target> and I<untrusted> parameters can be
B<NULL>. Yet note that if I<target> is NULL the verification target will need to
be set using X509_STORE_CTX_set_cert() before L<X509_verify_cert(3)> is invoked.
